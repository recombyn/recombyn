...
几分钟内用 Docker Compose 自托管（MySQL + Redis + Web + API）。可选 [Langfuse](https://langfuse.com) 做 Agent 追踪。MIT 协议 monorepo。
...
- **广场与项目** — 灵感流与作品（API）  
- **可观测性** — 可选本地 Langfuse（按 `metadata.task_id` 查）
...
> 上线前务必改掉默认数据库密码与超管 bootstrap 密码。  
> 完整说明：**[docs/self-hosting.md](docs/self-hosting.md)**
...
运营后台为**内部私有仓，不公开**。开源自托管只依赖 `apps/api/data/*.json` 种子即可启动。
