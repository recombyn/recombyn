...
class Settings(BaseSettings):
    ...
    # SQLModel / SQLAlchemy uses the same URL via app.core.db.sqlalchemy_database_uri().
    database_url: str = ""
    ...
    # Orphan paused checkpoint TTL (hours); 0 disables sweeper.
    design_run_checkpoint_ttl_hours: float = 72.0
    # How often the TTL sweeper wakes (hours).
    design_run_checkpoint_sweep_interval_hours: float = 6.0
    # Refuse InMemorySaver for design graph (prod). Set false for unit tests / ephemeral.
