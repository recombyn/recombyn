...
def get_library_item(*, session: Session, item_id: int) -> DesignLibraryItem | None:
    return session.get(DesignLibraryItem, int(item_id))
...
def list_library_items(
    *,
    ...
    offset: int = 0,
    limit: int = 24,
) -> tuple[list[DesignLibraryItem], int]:
    where: list[Any] = []
    if kind and kind.strip():
        where.append(DesignLibraryItem.kind == kind.strip())
    if scene and scene.strip() and scene.strip() != "all":
        scene_n = scene.strip()
        where.append(
            (DesignLibraryItem.scene == scene_n)
            | (DesignLibraryItem.scene == "all")
            | (col(DesignLibraryItem.scene).like(f"%{scene_n}%"))
        )
    if enabled is True:
        where.append(DesignLibraryItem.enabled == 1)
    elif enabled is False:
        where.append(DesignLibraryItem.enabled == 0)
    if q and q.strip():
        like = f"%{q.strip()}%"
        where.append(
            (col(DesignLibraryItem.name).like(like))
            | (col(DesignLibraryItem.tags).like(like))
            | (col(DesignLibraryItem.description).like(like))
        )
    count_stmt = select(func.count()).select_from(DesignLibraryItem)
    stmt = (
        select(DesignLibraryItem)
        .order_by(col(DesignLibraryItem.sort_order).asc(), col(DesignLibraryItem.id).desc())
        .offset(max(0, offset))
        .limit(max(1, limit))
    )
    if where:
...
def upsert_library_item(
    *,
    ...
    now = time.time()
    if item_id:
        row = get_library_item(session=session, item_id=int(item_id))
        if not row:
...
def soft_delete_library_item(*, session: Session, item_id: int) -> bool:
    row = get_library_item(session=session, item_id=item_id)
    if not row:
        return False
    row.enabled = 0
    row.updated_at = time.time()
...
def hard_delete_library_item(*, session: Session, item_id: int) -> bool:
    row = get_library_item(session=session, item_id=item_id)
    if not row:
        return False
    session.delete(row)
...
) -> DesignQualitySample | None:
    return session.get(DesignQualitySample, int(sample_id))
...
def list_quality_samples(
    *,
    ...
    offset: int = 0,
    limit: int = 24,
) -> tuple[list[DesignQualitySample], int]:
    where: list[Any] = []
    if scene and scene.strip() and scene.strip() != "all":
        where.append(DesignQualitySample.scene == scene.strip().lower())
    if grade and grade.strip() and grade.strip() != "all":
        where.append(DesignQualitySample.grade == grade.strip().lower())
    if enabled is True:
        where.append(DesignQualitySample.enabled == 1)
    elif enabled is False:
        where.append(DesignQualitySample.enabled == 0)
    if embed_status and embed_status.strip() and embed_status.strip() != "all":
        where.append(DesignQualitySample.embed_status == embed_status.strip().lower())
    if q and q.strip():
        like = f"%{q.strip()}%"
        where.append(
            (col(DesignQualitySample.name).like(like))
            | (col(DesignQualitySample.tags).like(like))
            | (col(DesignQualitySample.comment_text).like(like))
        )
    count_stmt = select(func.count()).select_from(DesignQualitySample)
    stmt = (
        select(DesignQualitySample)
        .order_by(
            col(DesignQualitySample.updated_at).desc(),
            col(DesignQualitySample.id).desc(),
        )
        .offset(max(0, offset))
        .limit(max(1, limit))
    )
...
def find_enabled_quality_sample_by_url(
    *, session: Session, image_url: str
) -> DesignQualitySample | None:
    return session.exec(
        select(DesignQualitySample)
        .where(DesignQualitySample.image_url == image_url)
        .where(DesignQualitySample.enabled == 1)
        .order_by(col(DesignQualitySample.id).desc())
        .limit(1)
    ).first()


def list_ready_quality_embeddings(
    *,
    session: Session,
    scene: str,
    grade: str,
    limit: int,
) -> list[DesignQualitySample]:
    return list(
        session.exec(
            select(DesignQualitySample)
            .where(DesignQualitySample.enabled == 1)
            .where(DesignQualitySample.embed_status == "ready")
            .where(DesignQualitySample.scene == scene)
            .where(DesignQualitySample.grade == grade)
            .where(col(DesignQualitySample.aesthetic_emb).is_not(None))
            .order_by(col(DesignQualitySample.updated_at).desc())
            .limit(max(1, limit))
        ).all()
    )


def count_ready_good_quality_by_scene(
    *, session: Session
) -> dict[str, int]:
    rows = session.exec(
        select(DesignQualitySample.scene, func.count())
        .where(DesignQualitySample.enabled == 1)
        .where(DesignQualitySample.embed_status == "ready")
        .where(DesignQualitySample.grade == "good")
        .where(col(DesignQualitySample.aesthetic_emb).is_not(None))
        .group_by(DesignQualitySample.scene)
    ).all()
    out: dict[str, int] = {}
...
def list_design_knowledge_keys(*, session: Session) -> set[tuple[str, str]]:
    rows = session.exec(select(DesignKnowledge.kind, DesignKnowledge.title)).all()
    return {(str(k or ""), str(t or "")) for k, t in rows}


def insert_design_knowledge_seed(
    *,
    session: Session,
    kind: str,
    title: str,
...
def list_design_knowledge(
    *,
    ...
    where: list[Any] = []
    if kind:
        where.append(DesignKnowledge.kind == kind)
    if enabled is not None:
        where.append(DesignKnowledge.enabled == (1 if enabled else 0))
    stmt = select(DesignKnowledge).order_by(
        col(DesignKnowledge.sort_order).asc(), col(DesignKnowledge.id).asc()
    )
    if where:
        stmt = stmt.where(*where)
    return list(session.exec(stmt).all())
...
def get_design_knowledge(*, session: Session, item_id: int) -> DesignKnowledge | None:
    return session.get(DesignKnowledge, int(item_id))


def upsert_design_knowledge(
    *,
    session: Session,
    item_id: int | None,
    kind: str,
    ...
    now = time.time()
    if item_id:
        row = get_design_knowledge(session=session, item_id=int(item_id))
        if not row:
        ...
        row.scenes = scenes
        row.skill_categories = skill_categories
        row.sort_order = sort_order
        row.enabled = enabled
        row.updated_at = now
    else:
...
def soft_delete_design_knowledge(*, session: Session, item_id: int) -> bool:
    row = get_design_knowledge(session=session, item_id=item_id)
    if not row:
        return False
    row.enabled = 0
    row.updated_at = time.time()
...
def count_quality_samples_by_grade(
    *, session: Session, scene: str
) -> list[Any]:
    return list(
        session.exec(
            select(DesignQualitySample.grade, func.count().label("c"))
            .where(DesignQualitySample.enabled == 1)
            .where(DesignQualitySample.embed_status == "ready")
            .where(col(DesignQualitySample.aesthetic_emb).is_not(None))
            .where(DesignQualitySample.scene == scene)
            .group_by(DesignQualitySample.grade)
        ).all()
    )


def count_design_library_items(*, session: Session) -> int:
    return int(
        session.exec(select(func.count()).select_from(DesignLibraryItem)).one() or 0
    )


def insert_design_library_seed_item(
    *,
    session: Session,
    name: str,
    kind: str,
...
def list_brush_library_items(*, session: Session) -> list[DesignLibraryItem]:
    return list(
        session.exec(
            select(DesignLibraryItem).where(DesignLibraryItem.kind == "brush")
        ).all()
    )


def list_enabled_brush_library_items(*, session: Session) -> list[DesignLibraryItem]:
    return list(
        session.exec(
            select(DesignLibraryItem)
            .where(DesignLibraryItem.kind == "brush")
            .where(DesignLibraryItem.enabled == 1)
            .order_by(
                col(DesignLibraryItem.sort_order).asc(),
                col(DesignLibraryItem.id).asc(),
            )
        ).all()
    )


def update_library_item_cover(
    *, session: Session, item_id: int, cover_url: str, updated_at: float
) -> None:
    row = session.get(DesignLibraryItem, int(item_id))
    if not row:
        return
    row.cover_url = cover_url
    row.updated_at = updated_at
