...
def palette_from_image_bytes(data: bytes, *, k: int = 6) -> list[str]:
    ...
    if not data:
        return []
    try:
        from app.services.design.aesthetics.views import load_pil
...
def analyze_sample_tokens(
    ...
    *,
    ...
    if meta is None and sid is not None:
        try:
            from app.services.design.admin.quality_sample_store import get_quality_sample
...
def build_aesthetic_token_guidance(
    *,
    ...
    try:
        from app.services.design.prompts.token_store import format_token_block, resolve_token_pack
