...
def seed_library_if_empty() -> int:
    ...
    with Session(engine) as session:
        if crud.count_design_library_items(session=session) > 0:
            return 0
        now = time.time()
        n = 0
        for item in _seed_items():
            ...
            crud.insert_design_library_seed_item(
                session=session,
                name=item["name"],
                kind=item["kind"],
                scene=item["scene"],
...
def refresh_brush_covers() -> int:
    ...
    with Session(engine) as session:
        rows = crud.list_brush_library_items(session=session)
        n = 0
        now = time.time()
        for r in rows:
            try:
            ...
            crud.update_library_item_cover(
                session=session,
                item_id=int(r.id or 0),
                cover_url=cover,
                updated_at=now,
            )
            n += 1
...
def list_public_brushes() -> list[dict[str, Any]]:
    ...
    from app.services.design.readpath.library_store import _pub

    ensure_design_catalog()
    ensure_library_seed()
    with Session(engine) as session:
        rows = crud.list_enabled_brush_library_items(session=session)
    out = []
    for r in rows:
        item = _pub(r)
