...
def embed_episode(episode_id: str) -> bool:
    ...
    try:
        ...
        with connect() as conn:
            row = conn.execute(
                "SELECT goal, summary FROM agent_episode WHERE id=?",
                (eid,),
            ).fetchone()
            if not row:
                return False
            ...
            conn.execute(
                """
                UPDATE agent_episode
                SET emb=?, emb_dim=?, emb_model=?, embed_status='ready', updated_at=?
                WHERE id=?
                """,
                (blob, int(EMB_DIM), MODEL_ID, time.time(), eid),
            )
            conn.commit()
        return True
    except Exception:
    ...
    *,
    ...
    try:
        ...
        with connect() as conn:
            conn.execute(
                """
                INSERT INTO agent_episode (
                    id, user_id, session_id, project_id, task_id, scene,
                    goal, summary, actions_json, observe_json, outcome,
                    emb, emb_dim, emb_model, embed_status, status,
                    created_at, updated_at
                ) VALUES (
                    ?, ?, ?, ?, ?, ?,
                    ?, ?, ?, ?, ?,
                    NULL, 0, '', 'pending', 'active',
                    ?, ?
                )
                """,
                (
                    ...
                    goal_t[:2000],
                    summary_t[:2000],
                    json.dumps(actions, ensure_ascii=False)[:8000],
        ...
        try:
            from services.agent_memory.kg import ingest_episode_graph
...
def _list_recency(uid: str, k: int) -> list[dict[str, Any]]:
    with connect() as conn:
        rows = conn.execute(
            """
            SELECT id, scene, goal, summary, actions_json, observe_json,
                   outcome, created_at
            FROM agent_episode
    ...
    return [
        ...
        for r in rows
        if str(r["goal"] or "").strip() or str(r["summary"] or "").strip()
    ]
...
def _list_by_embedding(uid: str, query: str, k: int) -> list[dict[str, Any]] | None:
    ...
    with connect() as conn:
        rows = conn.execute(
            """
            SELECT id, scene, goal, summary, actions_json, observe_json,
                   outcome, created_at, emb, embed_status
            FROM agent_episode
            WHERE user_id = ? AND status = 'active'
            ORDER BY created_at DESC
            LIMIT 200
    ...
    for r in rows:
        ...
        if vec is None:
            if status in ("pending", "", "failed"):
                pending_ids.append(str(r["id"] or ""))
            continue
