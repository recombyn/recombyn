    ...
    *,
    ...
    try:
        ...
        with connect() as conn:
            row = conn.execute(
                """
                SELECT id, weight FROM agent_kg_triple
            ...
            if row:
                ...
                conn.execute(
                    """
                    UPDATE agent_kg_triple
                    SET weight=?, source=?, updated_at=?
                    WHERE id=?
                    """,
                    (min(w, 99.0), (source or "episode")[:32], now, tid),
                )
                conn.commit()
                return tid
            ...
            conn.execute(
                """
                INSERT INTO agent_kg_triple (
                    id, user_id, subject, predicate, object, weight,
                ...
                (
                    ...
                    p,
                    o,
                    max(0.1, float(weight_delta)),
                    (source or "episode")[:32],
                    now,
                    now,
                ),
            )
...
def schedule_kg_skill_enrich(
    *,
    ...
    from services.agent_memory.text_embed import schedule_background
    ...
    *,
    ...
    try:
        ...
        with connect() as conn:
            rows = conn.execute(
                """
                SELECT subject, predicate, object, weight, updated_at
                FROM agent_kg_triple
...
def list_triples_admin(
    *,
        ...
        with connect() as conn:
            if uid:
                total_row = conn.execute(
                    """
                    SELECT COUNT(*) AS c FROM agent_kg_triple
                ...
                rows = conn.execute(
                    """
                    SELECT id, user_id, subject, predicate, object, weight,
                           source, status, created_at, updated_at
                    FROM agent_kg_triple
                    WHERE status='active' AND user_id=?
                ...
                total_row = conn.execute(
                    "SELECT COUNT(*) AS c FROM agent_kg_triple WHERE status='active'"
                ).fetchone()
                rows = conn.execute(
                    """
                    SELECT id, user_id, subject, predicate, object, weight,
    ...
    return {"items": items, "total": int((total_row or {}).get("c") or 0)}
