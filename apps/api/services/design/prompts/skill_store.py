...
def _load_user_skill_prefs(user_id: str) -> dict[str, bool]:
    ...
    with connect() as conn:
        rows = conn.execute(
            "SELECT skill_key, enabled FROM design_user_skill_pref WHERE user_id = ?",
            (uid,),
        ).fetchall()
    out: dict[str, bool] = {}
    for r in rows:
...
def list_my_skills(*, user_id: str) -> list[dict[str, Any]]:
    ...
    with connect() as conn:
        rows = conn.execute(
            """
            SELECT * FROM design_skill
            WHERE owner_user_id = ? AND namespace = ?
...
def set_user_skill_enabled(
    *, user_id: str, skill_id: int, enabled: bool
    ...
    with connect() as conn:
        row = conn.execute(
            "SELECT * FROM design_skill WHERE id = ?", (sid,)
        ).fetchone()
        if not row:
        ...
        if mine:
            conn.execute(
                """
                UPDATE design_skill SET enabled = ?, updated_at = ?
                WHERE id = ? AND owner_user_id = ?
                """,
                (1 if enabled else 0, now, sid, uid),
            )
        # Pref overlay (portable upsert — sqlite / mysql / postgres).
        pref = conn.execute(
            """
            SELECT id FROM design_user_skill_pref
            WHERE user_id = ? AND skill_key = ? LIMIT 1
            """,
            (uid, key),
        ).fetchone()
        ...
        if pref:
            ...
            conn.execute(
                """
                INSERT INTO design_user_skill_pref
                  (user_id, skill_key, enabled, updated_at)
                VALUES (?, ?, ?, ?)
                """,
                (uid, key, flag, now),
            )
...
def upsert_end_user_skill(*, user_id: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    with connect() as conn:
        if sid:
            row = conn.execute(
                "SELECT id, skill_key, owner_user_id, version FROM design_skill WHERE id = ?",
                (sid,),
            ).fetchone()
            if not row:
                raise ValueError("skill not found")
            ...
            conn.execute(
                ...
                UPDATE design_skill SET
                  name=?, category=?, prompt_positive=?, prompt_negative=?,
                  when_to_use=?, description=?, logo=?, triggers=?,
            ...
            if meta_errs:
                raise ValueError("; ".join(meta_errs))
            exists = conn.execute(
                "SELECT id FROM design_skill WHERE skill_key = ? LIMIT 1",
                (skill_key,),
            ).fetchone()
            if exists:
            ...
            cur = conn.execute(
                ...
                INSERT INTO design_skill (
                    skill_key, name, category, prompt_positive, prompt_negative,
                    when_to_use, preferred_tools, allowed_resources, triggers,
                    mutex_group, version, source, namespace, owner_user_id,
                    description, logo, locales, sort_weight, scenes,
                    default_model, max_retries, enabled, output_format,
                    allow_user_model_override, created_at, updated_at
                ) VALUES (
                    ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, 1, ?, ?, ?, ?, ?, '{}', 0, 'all',
                    'doubao', 2, ?, 'json', 0, ?, ?
                )
            ...
            saved = conn.execute(
                "SELECT * FROM design_skill WHERE id = ?", (new_id,)
            ).fetchone()
    invalidate_skill_key_cache()
    if not saved:
...
def delete_end_user_skill(*, user_id: str, skill_id: int) -> bool:
    ...
    with connect() as conn:
        row = conn.execute(
            "SELECT owner_user_id, source, namespace FROM design_skill WHERE id = ?",
            (int(skill_id),),
        ).fetchone()
        if not row:
            return False
        ...
        cur = conn.execute(
            "DELETE FROM design_skill WHERE id = ? AND owner_user_id = ?",
            (int(skill_id), uid),
        )
        conn.commit()
...
def _fetch_owned_skill_by_key(conn: Any, *, uid: str, key: str) -> Any | None:
    if not key:
        return None
    return conn.execute(
        """
        SELECT * FROM design_skill
        WHERE owner_user_id = ? AND skill_key = ?
        LIMIT 1
        """,
        (uid, key),
    ).fetchone()
...
def _fetch_owned_skill_by_name(conn: Any, *, uid: str, name_l: str) -> Any | None:
    if not name_l:
        return None
    return conn.execute(
        """
        SELECT * FROM design_skill
        WHERE owner_user_id = ? AND LOWER(name) = ?
        ORDER BY updated_at DESC, id DESC
        LIMIT 1
        """,
        (uid, name_l),
...
def _fetch_foreign_skill_by_key(conn: Any, *, uid: str, key: str) -> Any | None:
    if not key:
        return None
    other = conn.execute(
        "SELECT * FROM design_skill WHERE skill_key = ? LIMIT 1",
        (key,),
    ).fetchone()
    if not other:
        return None
...
def _find_owned_skill_conflict(
    *, user_id: str, skill_key: str, name: str
    ...
    ensure_design_skills()
    with connect() as conn:
        row = _fetch_owned_skill_by_key(conn, uid=uid, key=key)
        if not row:
            row = _fetch_owned_skill_by_name(conn, uid=uid, name_l=name_l)
        if row:
            return _conflict_card(row, mine=True)
        foreign = _fetch_foreign_skill_by_key(conn, uid=uid, key=key)
        if foreign:
...
def _apply_pack_version(*, uid: str, skill_id: int, pack_ver: str | None) -> None:
    ...
    try:
        with connect() as conn:
            conn.execute(
                "UPDATE design_skill SET pack_version = ? WHERE id = ? AND owner_user_id = ?",
                (pack_ver, skill_id, uid),
            )
            conn.commit()
    except Exception:
