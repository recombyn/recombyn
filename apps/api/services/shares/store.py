...
def _reuse_share_acl_fields(
    ...
    *,
    ...
    if keep_acl:
        return (
            str(existing["permission"] or "preview"),
...
def _find_existing_project_share(conn: Any, owner_id: str, source_project_id: str) -> Any:
    return conn.execute(
        """
        ...
        LIMIT 1
        """,
        (owner_id, source_project_id),
    ).fetchone()


def _reload_share(conn: Any, share_id: str, *, actor_user_id: str) -> dict[str, Any]:
    row = conn.execute(
...
def create_share(
    *,
    ...
    with connect() as conn:
        # Reuse per-project share so reopening the dialog does not wipe collaborators.
        if src:
            existing = _find_existing_project_share(conn, uid, src)
            if existing:
                sid = str(existing["id"])
                ...
                next_enabled = 1 if _link_enabled(existing) else 0
                conn.execute(
                    """
                    UPDATE document_shares
                    SET name = ?, document_json = ?, permission = ?,
                        editor_user_ids = ?, viewer_user_ids = ?,
                        link_enabled = ?, link_public = ?, updated_at = ?
                    WHERE id = ?
                    """,
                    (
                        title,
                ...
                conn.commit()
                return _reload_share(conn, sid, actor_user_id=uid)
        ...
        conn.execute(
            ...
            INSERT INTO document_shares (
                ...
                source_project_id, editor_user_ids, viewer_user_ids,
                link_enabled, link_public, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                sid,
...
def update_share_meta(
    ...
    *,
    ...
    with connect() as conn:
        ...
        title = str(row["name"] or "Untitled")
        if name is not None:
            title = _clamp_share_title(name)
        ...
        conn.execute(
            """
            UPDATE document_shares
            SET permission = ?, editor_user_ids = ?, viewer_user_ids = ?,
                name = ?, link_enabled = ?, link_public = ?, updated_at = ?
            WHERE id = ?
            """,
            (
                perm,
        ...
        conn.commit()
        return _reload_share(conn, sid, actor_user_id=actor)
