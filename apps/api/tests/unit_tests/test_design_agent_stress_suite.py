# -*- coding: utf-8 -*-
"""Validate design_agent_stress_suite.json shape + skill_expect keys exist on disk."""
...
def _suite() -> dict:
    path = api_seeds_dir() / "design_agent_stress_suite.json"
    assert path.is_file(), f"missing stress suite: {path}"
...
def test_stress_suite_system_cases_shape():
    suite = _suite()
...
def test_stress_suite_skill_expect_exist_on_disk():
    skills = _skill_dirs()
