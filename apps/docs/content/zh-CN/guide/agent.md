...
| **第三方自定义模型** | 走自有 Key 与端点；不消耗平台积分（见 [第三方模型](#第三方大模型自有-key)） |

免费档通常仅可使用 **Auto**；标准档及以上可自选平台模型。具体以账户「方案」页为准。
...
部分会员方案（如标准档起）可在 **账户 → Agent → 第三方模型** 添加兼容 **OpenAI / Claude API** 的供应商。

### 怎么添加

1. 打开账户设置 → **Agent**。
2. 在「第三方模型」区域填写：
   - **模型类型**：**对话**（聊天改稿）或 **多模态**（可看图）。生图请继续用平台生图模型（消耗平台积分）。
   - **供应商名称**、可选官网链接
   - **API Key**（仅保存在本机浏览器，不会作为平台密钥入库）
   - **请求地址**（兼容端点，不要以 `/` 结尾）
3. 保存后，模型会出现在对话区的模型列表中，可直接选用。

### 计费注意

- 使用第三方模型时，**调用走你自己的额度**，**不扣平台积分**。
- 同一会话里若 Agent 仍调用平台生图 / 图片工具，那些步骤仍按平台规则扣积分。
- Key 与端点仅本机保存；清除浏览器数据会丢失本地配置，需重新添加。

未开通会员时，该区域会提示升级；可在方案页开通后再配置。
