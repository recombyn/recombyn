...
| **第三方自訂模型** | 使用自有 Key 與端點；**不消耗**平台積分（見 [第三方大模型](#第三方大模型自有-key)） |
...
部分會員方案可在 **帳戶 → Agent → 第三方模型** 新增相容 **OpenAI / Claude API** 的供應商。

### 怎麼新增

1. 開啟帳戶設定 → **Agent**。
2. 填寫：
  - **模型類型**：**對話** 或 **多模態**。生圖請用平台生圖模型（消耗平台積分）。
  - 供應商名稱、可選官網
  - **API Key**（僅保存在本機瀏覽器）
  - **請求位址**（相容端點，不要以 `/` 結尾）
3. 儲存後，模型會出現在對話區模型列表。
...
- 第三方聊天呼叫走**你自己的額度**，**不扣平台積分**。
- 同一工作階段若仍呼叫平台生圖 / 圖片工具，仍依平台規則扣積分。
- 清除瀏覽器資料會遺失本機 Key，需重新新增。
