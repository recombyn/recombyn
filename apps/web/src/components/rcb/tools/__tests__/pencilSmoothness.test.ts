...
describe('pencil stroke smoothness', () => {
  ...
  it('simplifyPencilCenterline drops colinear points and keeps pressure', () => {
    ...
    // Near-colinear: small epsilon collapses middle points.
  ...
  it('freehand outline stays centered on sharp polyline', async () => {
    const { outlinePathFromPoints } = await import('../pencilBrushes');
    const pts: Pt[] = [
      { x: 0, y: 10 },
      { x: 40, y: 10 },
      { x: 40, y: 50 },
    ...
    // Ray-cast: centerline samples must sit inside the silhouette (not above a bowed stroke).
