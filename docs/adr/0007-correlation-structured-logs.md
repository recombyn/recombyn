...
4. **OTel:** deferred until we need cross-service spans (collab ↔ API) or a collector; this ADR does **not** add OTel deps.
...
- Not W3C `traceparent` / OTel-compatible until a later ADR.
- Metrics stay low-cardinality (no `trace_id` label).

## Alternatives considered

1. **OTel SDK now** — rejected as first slice; high install/config cost.
