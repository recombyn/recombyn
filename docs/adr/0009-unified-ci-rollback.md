...
Phase 1–3 left us with path-filtered workflows (web / api / e2e / perf) but no single PR gate for `lint → typecheck → unit`. Full web `tsc` is still dirty; e2e and Docker publish are heavier. We need a required-style umbrella without inventing k8s or blocking on dirty `tsc`.

## Decision

1. **Umbrella workflow** `.github/workflows/ci.yml` on every PR / push to main:
   - `check` — `npm run check` (web ESLint + contracts typecheck)
...
- Local mirror: `npm run ci:gate` (check + web unit; API unit via existing script).
