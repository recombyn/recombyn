...
Shipped Design Agent skills lived under `apps/api/seeds/design_skills`. The open catalog should match the public monorepo shape (`skills/foundation` + `skills/domains`) while keeping private deploy mounts separate.
...
- Matches the public monorepo mental model.
...
1. **Keep only seeds/** — rejected; fights the agreed public tree.
