...
When **Recombyn Intelligence** is configured (`RECOMBYN_INTELLIGENCE_URL`), the API BFF calls
`POST /api/v1/pipeline/analyze-pages` on intelligence (same OCR/layout stack, closed-source).
Local PaddleOCR in the web API is only used as fallback when intelligence fails and `.[ocr]` is installed.
...
See [vision-intelligence.md](./vision-intelligence.md).
...
> **Note:** Image toolbar features (抠图 / 分层 / 编辑文字) are **not** part of import. They require Recombyn Intelligence via `RECOMBYN_INTELLIGENCE_URL` — see `apps/api/.env.example`.

The frontend shows "Importing…" during import.

## Stage 6: integration readiness

- `GET /api/v1/health` reports `redis` / `worker` / `intelligence` status
