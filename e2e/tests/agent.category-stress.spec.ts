/**
 * Browser stress: category craft flows (poster / banner / mobile UI / dashboard / landing / …).
 * Prompts from apps/api/seeds/design_agent_stress_suite.json.
 *
 * Stall rule (suite note): >150s with no canvas growth → Stop and retry once.
 *
 * Requires: web (E2E_BASE_URL, prefer http://localhost:3000), API, token in ../.tmp-token.txt
 */
...
function selectedCases(): SuiteCase[] {
  ...
  const raw = (process.env.E2E_STRESS_CASES || '').trim();
  const ids = raw
  ...
  if (!picked.length) {
    throw new Error(`No stress cases matched: ${[...want].join(',')}`);
  }
  return picked;
}
...
async function sendPrompt(page: Page, prompt: string) {
  ...
  // fill is faster/more reliable than per-char type for long stress prompts.
